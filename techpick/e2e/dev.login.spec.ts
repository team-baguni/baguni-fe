    ...
    test('회원 가입 성공 후 회원 탈퇴 테스트', async ({ page, request }) => {
        ...
        const [response] = await Promise.all([
            page.waitForResponse(res =>
                res.url().includes('/api/development/users/new/signup') &&
        ...
        const deleteResponse = await request.delete(`${apiUrl}/api/development/users?userId=${userId}`);

        // 검증
        expect(page.locator('text=회원가입이 되었습니다. 로그인을 해주세요.')).toBeVisible(); // toast 메시지 검증
